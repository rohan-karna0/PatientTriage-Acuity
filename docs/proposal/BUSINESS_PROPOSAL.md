...
Acuity is validated with a **35-case golden benchmark** (synthetic, India-relevant vignettes):

```bash
npm test && npm run evaluate
```

Targets: **0% under-triage proxy** on critical cases, **100%** ESI range match, escalation bias, route sanity, and surge escalation. Full methodology: [technical/EVALUATION.md](../technical/EVALUATION.md). Latest results: [technical/EVALUATION_RESULTS.md](../technical/EVALUATION_RESULTS.md).

## 10. Tech stack

- **Frontend:** Next.js 15, TypeScript, clinical light UI (DOOR / FLOW / WATCH)
- **Backend:** Next.js API routes, Prisma, SQLite (demo) / Postgres (production)
- **Engine:** `@acuity/triage-engine` — age-stratified hybrid ESI scorer (Vitest)
- **Monorepo:** `apps/web`, `packages/triage-engine`, `packages/shared`

## 11. Deliverables map

| Round 2 deliverable | Repository path |
|---|---|
| Documentation index | `docs/README.md` |
| Business proposal | `docs/proposal/BUSINESS_PROPOSAL.md` |
| Architecture | `docs/technical/ARCHITECTURE.md` |
| Data flow | `docs/technical/DATA_FLOW.md` |
| Compliance | `docs/compliance/COMPLIANCE.md` |
| Evaluation | `docs/technical/EVALUATION.md` + `data/benchmark/golden-cases.json` |
| Evaluation results | `docs/technical/EVALUATION_RESULTS.md` + `data/benchmark/benchmark-report.json` |
| Demo script | `docs/demo/DEMO_SCRIPT.md` |
| User guide | `docs/demo/USER_GUIDE.md` |
| Runnable prototype | `apps/web` — `npm run setup && npm run dev` |
| Submission checklist | `docs/submission/SUBMISSION.md` |
